# =============================================================================
# app.R  -  Pretest Simulation Studio UI + server (package internals)
# -----------------------------------------------------------------------------
# app_ui() and app_server() drive the ORIGINAL framework's run_simulation()
# engine through its six phases. They are wired together by run_app().
# All Shiny / bslib / DT functions are imported via the package NAMESPACE.
# =============================================================================
...
# Theme builder (lazy: built at UI construction so font fetch is deferred).
...
#' @return A bslib page_navbar UI object.
#' @keywords internal
app_ui <- function() {
...
app_server <- function(input, output, session) {
  ...
  detect_fw <- function() {
    # 1) option override or the copy bundled with the package
    d <- default_framework_path()
    if (!is.na(d)) return(d)
    # 2) fall back to common on-disk locations (running from source, etc.)
