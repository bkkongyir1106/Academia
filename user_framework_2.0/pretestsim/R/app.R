...
build_theme <- function() {
  bslib::bs_add_rules(
    ...
    .metric-value,
    .metric-value .shiny-text-output {
      font-size: clamp(1.25rem, 1.75vw, 1.65rem) !important;
      font-weight: 700;
      line-height: 1;
    }
    .metric-strip .card,
    .metric-strip .bslib-value-box,
    .metric-strip .value-box {
      min-height: 96px;
    }
    .metric-strip .card-body,
    .metric-strip .value-box-area {
      padding: 0.85rem 1rem !important;
    }
    .metric-strip .value-box-showcase {
      font-size: 2rem !important;
      width: 3.25rem !important;
    }
    .metric-strip .value-box-title {
      font-size: 0.9rem !important;
      line-height: 1.15;
    }
    .metric-strip .value-box-value,
    .metric-strip .shiny-text-output {
      font-size: 1.65rem !important;
      line-height: 1.05;
    }
    .phase-side-table .card-body,
    .phase-side-table .card-body > div {
      min-width: 0;
    }
    .auc-scroll {
      max-height: 640px;
...
app_ui <- function(raw_base = .default_raw_base()) {
  bslib::page_navbar(
    ...
    bslib::nav_panel("Phase 1", bslib::layout_columns(col_widths = c(9, 3), class = "phase-grid-wide",
      bslib::card(bslib::card_header("Normality ROC"), plotOutput("p1_plot", height = 560)),
      bslib::card(class = "phase-side-table",
                  bslib::card_header("AUC table"),
                  div(class = "auc-scroll", DT::DTOutput("p1_auc")))
    )),
    bslib::nav_panel("Phase 2",
      div(class = "metric-strip",
        bslib::layout_columns(col_widths = c(3, 3, 3, 3),
          div(class = "metric-tile primary",
              div(class = "metric-title", "Threshold"),
              div(class = "metric-value", textOutput("p2_star", inline = TRUE))),
          div(class = "metric-tile success",
              div(class = "metric-title", "Power gain"),
              div(class = "metric-value", textOutput("p2_gain", inline = TRUE))),
          div(class = "metric-tile",
              div(class = "metric-title", "Power loss"),
              div(class = "metric-value", textOutput("p2_loss", inline = TRUE))),
          div(class = "metric-tile",
              div(class = "metric-title", "Type I inflation"),
              div(class = "metric-value", textOutput("p2_infl", inline = TRUE)))
        )
      ),
      bslib::card(bslib::card_header("Trade-off"), plotOutput("p2_plot", height = 600))
    ),
    bslib::nav_panel("Phase 3", bslib::card(bslib::card_header("Power vs Type I error"), plotOutput("p3_plot", height = 520))),
    bslib::nav_panel("Phase 4", bslib::layout_columns(col_widths = c(8, 4), class = "phase-grid-wide",
      bslib::card(bslib::card_header("Power and Type I vs n"), plotOutput("p4_plot", height = 560)),
      bslib::card(class = "phase-side-table",
                  bslib::card_header("AUC tables"),
                  div(class = "auc-scroll", uiOutput("p4_tables")))
    )),
