...
### WORKING DIRECTORY
## ============================================================#
setwd("~/Desktop/OSU/Research Work/User_framework_rpkg")
...
# 
# source(
#   file  = "ML_Approach_application_v2.R",
#   local = ml_env
# )
# 
# stopifnot(exists("prepare_prediction_features", envir = ml_env))
# stopifnot(exists("predict_prob_normal_fast", envir = ml_env))
# 
...
onesample_data <- function(n = 10, effect_size = 0.0, sd = 1, dist = "Exponential", par = NULL, center_by = "median",...) {
  dist <- tolower(dist)
...
#' ----------------------------------------------------------------
#' (vi)  Randomized two-sided sign-test p-value.
#' Under H0, the exact sign-test distribution is broken by an
#' independent uniform draw so that the resulting p-value is Uniform(0, 1).
randomized_sign_pvalue <- function(data, mu0 = 0) {
  n_obs   <- length(data)
  s_obs   <- sum(data > mu0)
  dev_obs <- abs(s_obs - n_obs / 2)
  
  counts      <- 0:n_obs
  count_dev   <- abs(counts - n_obs / 2)
  count_probs <- dbinom(counts, n_obs, 0.5)
  
  p_strict <- sum(count_probs[count_dev > dev_obs])
  p_equal  <- sum(count_probs[count_dev == dev_obs])
  
  return(list(p.value = p_strict + runif(1) * p_equal))
}

#' -----------------------------------------------------------------
#' (vii) One-Sample Permutation Test using t-test statistic
one_sample_perm_test <- function(data, mu0 = 0, nresample = 1000) {
  n <- length(data)
  
  # Manual t-statistic (algebraically identical to t.test(data, mu = mu0)$statistic)
  ...
  return(list(p.value = p, statistic = observe_stat))
}

#' -----------------------------------------------------------------
#' (viii) One-Sample Bootstrap-t Test using t-test statistic
one_sample_boot_test <- function(data, mu0 = 0, nresample = 1000) {
  ...
  p_value <- (sum(abs(boot_stat) >= abs(t_obs), na.rm = TRUE) + 1) / (nresample + 1)
  
  return(list(p.value = p_value, statistic = t_obs))
}

#' -----------------------------------------------------------------
#'(ix) Bootstrap Median Test
bootstrap_median_test <- function(data, mu = 0, nresample = 1000) {
  data <- data[!is.na(data)]
  n <- length(data)
  obs_median <- median(data)
  mad_est <- mad(data)  # or bootstrap SE of the median
  t_obs <- (obs_median - mu) / mad_est
  
  centered_data <- data - obs_median + mu
  boot_stat <- replicate(nresample, {
    x_star <- sample(centered_data, n, replace = TRUE)
    (median(x_star) - mu) / mad(x_star)
  })
...
#' -----------------------------------------------------------------
#' (x) Box-Cox transformation helper
#' Shifts data (and mu) to be positive, estimates lambda via profile
#' log-likelihood, and returns the transformed data/mu/lambda/shift.
#' Shared by both boxcox_t_test() and boxcox_boot_test().
#' -----------------------------------------------------------------
...
#' -----------------------------------------------------------------
#' (x-a) Box-Cox transformed t-test
#' -----------------------------------------------------------------
boxcox_t_test <- function(data, mu = 0) {
  ...
  test_result <- tryCatch(
    t.test(bc$transformed_data, mu = bc$transformed_mu),
...
#' -----------------------------------------------------------------
#' (x-b) Box-Cox transformed t-test — bootstrap version
#' Re-estimates lambda on every resample, so the p-value reflects
#' lambda's estimation uncertainty rather than treating it as fixed.
#' -----------------------------------------------------------------
boxcox_boot_test <- function(data, mu = 0, nresample = 1000) {
  data <- as.numeric(data)
  data <- data[is.finite(data)]
  n <- length(data)
  
  if (n < 3 || sd(data) == 0) return(list(p.value = NA_real_))
  
  obs <- boxcox_t_test(data, mu)
  if (is.na(obs$p.value)) return(list(p.value = NA_real_))
  t_obs <- obs$statistic
  
  # Recenter on the ORIGINAL scale so resamples satisfy the null in mean.
  x0 <- data - mean(data) + mu
  
  boot_stat <- replicate(nresample, {
    x_star <- sample(x0, size = n, replace = TRUE)
    bc <- boxcox_transform_data(x_star, mu)
    ...
    if (is.null(test_result)) NA_real_ else unname(test_result$statistic)
  })
  ...
  list(
    ...
    statistic    = t_obs,
    lambda       = obs$lambda,
    n_valid_boot = sum(!is.na(boot_stat)),
    method       = "Bootstrap Box-Cox transformed one-sample t-test"
  )
}
...
plot_normality_roc_wrapper <- function(norm_config,
  ...
  if (method == "custom") {
    ...
    #' For multi-curve custom tests (e.g. ML), use the rownames
    ...
    plot_norm_roc_curve(curves    = curves, dist_name = distributions[1])
...
#' tests t1, and t2 p-values under H0 and H1 across Nsim replicates
#' --------------------------------------------------------------------------
...
#'
#' Stage 1 (preferred): among thresholds whose worst-case positive Type I
#' error inflation is within tol_pos, maximize power gain subject to power
#' loss <= loss_tol.
#'
#' Stage 2 (fallback): if no threshold satisfies the inflation tolerance
#' (possible when a user-supplied grid omits endpoints 0 and 1, or because
#' simulation/custom metrics make all estimated inflations exceed tol_pos),
#' minimize worst-case positive inflation first, then apply the same power criterion.
#' =============================================================================#
...
select_optimal_parameter <- function(distributions = c("exponential", "normal"),
  ...
  #' --------------------------------------------------------------------------
  #' Validate lengths: param_grid and all metric vectors must match
  #' --------------------------------------------------------------------------
  input_lengths <- c(
  ...
  if (length(unique(input_lengths)) != 1L) {
    stop(
      "param_grid and all metric vectors must have identical lengths. ",
  ...
  #' --------------------------------------------------------------------------#
  #' Filter out non-finite values before comparisons
  #' --------------------------------------------------------------------------#
  #' This guard protects against undefined estimates from failed test calls,
  #' degenerate samples, undefined resampling statistics, or user-supplied
  #' custom routines that return NA/NaN/Inf for some thresholds.
  ...
  #' --------------------------------------------------------------------------#
  #' Type I error inflation summary
  #' --------------------------------------------------------------------------#
  ...
  ## Endpoint indicators are recorded because including 0 and 1 is expected
  ## to make the controlled set non-empty under the endpoint-routing convention.
  ## Users may still supply a custom grid without these endpoints, so the
  ## fallback below is necessary
  ...
  #' --------------------------------------------------------------------------#
  #' Power-based selection within a supplied candidate set
  #' --------------------------------------------------------------------------#
  select_best <- function(candidates) {

    if (length(candidates) == 0L) {
      stop("select_best() received an empty candidate set.")
    }

    loss_vals <- power_loss[candidates]

    ## Prefer candidates whose power loss is within tolerance.
    ...
    if (length(good_loss) > 0L) {
      ...
      ## If no candidate meets the loss tolerance, minimize loss first.
    ...
    ## Deterministic tie-break: choose the smallest threshold value.
  ...
  ## Stage 1: retain thresholds whose worst-case positive Type I error
  ## inflation is within the tolerated amount.
  controlled_idx <- which(worst_pos_infl <= tol_pos + eps)

  if (length(controlled_idx) > 0L) {

    candidate_idx <- controlled_idx
    optimal_index <- select_best(candidate_idx)

    selection_stage <- 1L
    selection_note  <- sprintf(
      "Stage 1: worst-case positive Type I error inflation within tolerance (<= %.3g)",
      tol_pos
    )
    ...
    ## Stage 2 fallback: possible when the user supplies a threshold grid
    ## without endpoint values 0 and 1, or when simulation/custom metrics make
    ## all estimated inflations exceed tol_pos. Minimize inflation first, then
    ## apply the same power criterion.
    ...
    selection_note  <- if (has_both_endpoints) {
      sprintf(
        "Stage 2 fallback: endpoints were present but no threshold met tol_pos; minimized worst-case positive inflation (min = %.3g)",
        min_infl
      )
    } else {
      sprintf(
        "Stage 2 fallback: no threshold met tol_pos, possibly because the user grid did not include both endpoints 0 and 1; minimized worst-case positive inflation (min = %.3g)",
        min_infl
      )
    }
  ...
  list(
    ...
    optimal_index         = optimal_index,  # retained for backward compatibility
    ...
    feasible              = inflation_controlled,  # retained for backward compatibility
...
power_vs_error_roc_data <- function(N                  = 1e3,
  ...
  #' ---------------------------------------------------------------------------
  #' Attach settings for verification and downstream use by the plot function
  #' ---------------------------------------------------------------------------
...
#'
#' For each distribution and sample size, this function calls generate_pval()
#' once, then reuses the generated downstream p-values and normality decisions
#' to compute power and Type I error for test_1, test_2, and the adaptive test.
#'
#' This avoids unnecessary recalculation of:
#'   1. simulated datasets,
#'   2. downstream test p-values,
#'   3. normality/pretest decisions,
#'   4. adaptive-test p-values.
#' =============================================================================
...
run_ds_simulation <- function(sample_sizes       = c(10, 20, 30, 40, 50),
  ...
  ds_test_methods <- match.arg(
    ds_test_methods,
    choices    = valid_methods,
    several.ok = TRUE
  )
  
  ## Keep methods in a stable order.
  ds_test_methods <- valid_methods[valid_methods %in% ds_test_methods]
  
  #' ---------------------------------------------------------------------------
  #' pretest_threshold resolution — three-tier priority:
  #'   1. threshold_by_n, named vector, one value per n
  #'   2. optimal_result$param_star, single optimal threshold from Phase 2
  #'   3. pretest_threshold scalar, fallback default
  #' ---------------------------------------------------------------------------
  use_per_n <- !is.null(threshold_by_n) &&
    is.numeric(threshold_by_n) &&
    !is.null(names(threshold_by_n))
  ...
  if (!use_per_n) {
    if (!is.null(optimal_result) && !is.null(optimal_result$param_star)) {
      ...
      warning(sprintf(
        "No optimal_result supplied. Using pretest_threshold = %.4f.",
        pretest_threshold
      ))
    }
  } else {
  ...
  #' ---------------------------------------------------------------------------
  #' Helper: determine whether a replicate uses the parametric test
  #' ---------------------------------------------------------------------------
  use_parametric_test <- function(decision_values, threshold) {
    decision_values <- as.numeric(decision_values)
    
    ## If normality assessment failed completely, do not default to test_1.
    ## This is safer than all(..., na.rm = TRUE), which can return TRUE
    ## when all values are NA.
    if (length(decision_values) == 0 || all(is.na(decision_values))) {
      return(FALSE)
    }
    
    all(decision_values > threshold, na.rm = TRUE)
  }
  ...
  for (dist in distributions) {
    ...
    for (i in seq_along(sample_sizes)) {
      ...
      #' -----------------------------------------------------------------------
      #' Resolve threshold for this sample size
      #' -----------------------------------------------------------------------
      n_threshold <- if (use_per_n) {
        tv <- threshold_by_n[as.character(n)]
        
        if (length(tv) == 0 || is.na(tv)) {
          warning(sprintf(
            "No threshold_by_n entry for n = %d. Falling back to pretest_threshold = %.4f.",
            n, pretest_threshold
          ))
          pretest_threshold
        } else {
      ...
      dist_results$thresholds_used[i] <- n_threshold
      
      start_time <- Sys.time()
      
      #' -----------------------------------------------------------------------
      #' Generate all downstream p-values and normality decisions ONCE.
      #' No data are regenerated inside this function after this point.
      #' -----------------------------------------------------------------------
      sim_pvals <- generate_pval(
        Nsim               = N,
        n                  = n,
        effect_size_H1     = effect_size_H1,
        effect_size_H0     = effect_size_H0,
      ...
      #' -----------------------------------------------------------------------
      #' Build adaptive p-values from stored normality decisions
      #' -----------------------------------------------------------------------
      use_test1_H0 <- vapply(
        sim_pvals$norm_decision_H0,
        use_parametric_test,
        logical(1),
      ...
      adaptive_H0 <- ifelse(
        use_test1_H0,
        sim_pvals$pval_ds_test1_H0,
        sim_pvals$pval_ds_test2_H0
      )
      
      adaptive_H1 <- ifelse(
        use_test1_H1,
        sim_pvals$pval_ds_test1_H1,
        sim_pvals$pval_ds_test2_H1
      )
      
      #' -----------------------------------------------------------------------
      #' Collect p-values by method
      #' -----------------------------------------------------------------------
      pval_H0_list <- list(
        test_1   = sim_pvals$pval_ds_test1_H0,
        test_2   = sim_pvals$pval_ds_test2_H0,
        adaptive = adaptive_H0
      )
      
      pval_H1_list <- list(
        test_1   = sim_pvals$pval_ds_test1_H1,
        test_2   = sim_pvals$pval_ds_test2_H1,
        adaptive = adaptive_H1
      )
      ...
      for (method in ds_test_methods) {
        ...
        dist_results$type1[[method]][i] <- mean(
          pval_H0_list[[method]] < alpha,
          na.rm = TRUE
        )
        
        dist_results$power[[method]][i] <- mean(
          pval_H1_list[[method]] < alpha,
          na.rm = TRUE
        )
        
        dist_results$pvals$H0[[method]][[i]] <- pval_H0_list[[method]]
        dist_results$pvals$H1[[method]][[i]] <- pval_H1_list[[method]]
      }
      
      dist_results$timing[[paste0("n=", n)]] <- difftime(
        Sys.time(),
        start_time,
        units = "secs"
      )
    ...
    for (method in ds_test_methods) {
      ...
      type1_df[[method]] <- dist_results$type1[[method]]
    }
    
    ## Put columns in the familiar order:
    ## n, test_1, test_2, adaptive, Distribution
    power_df <- power_df[, c("n", ds_test_methods, "Distribution"), drop = FALSE]
    type1_df <- type1_df[, c("n", ds_test_methods, "Distribution"), drop = FALSE]
...
#'
#' For each distribution and effect size, this function calls generate_pval()
#' once, then reuses the generated downstream p-values and normality decisions
#' to compute power for test_1, test_2, and the adaptive strategy.
#'
#' This avoids unnecessary recalculation caused by simulating separately for
#' test_1, test_2, and adaptive.
#' ==============================================================================
...
perform_ds_power_by_effect <- function(fixed_n           = 10,
  ...
  use_parametric_test <- function(decision_values, threshold) {
    ...
    ## If normality assessment failed completely, do not default to test_1.
    ## This avoids the problem that all(..., na.rm = TRUE) may return TRUE
    ## when all decision values are NA.
    if (length(decision_values) == 0 || all(is.na(decision_values))) {
      return(FALSE)
    }
  ...
  for (dist in distributions) {
    ...
    results_power <- setNames(
      vector("list", length(ds_test_methods)),
      ds_test_methods
    )
    ...
    for (method in ds_test_methods) {
      results_power[[method]]       <- numeric(length(effect_sizes))
      names(results_power[[method]]) <- paste0("d=", effect_sizes)
    ...
    for (i in seq_along(effect_sizes)) {
      ...
      #' -----------------------------------------------------------------------
      #' Build effect_size argument with the correct structure
      #' -----------------------------------------------------------------------
      ...
      current_effect_H0 <- zero_like(current_effect)
      
      #' -----------------------------------------------------------------------
      #' KEY CHANGE:
      #' Generate p-values and normality decisions once for this effect size.
      #' We only need the H1 side for power, but generate_pval() returns both
      #' H0 and H1. The H0 output is ignored here.
      #' -----------------------------------------------------------------------
      sim_pvals <- generate_pval(
        Nsim               = Nsim,
        n                  = fixed_n,
        effect_size_H1     = current_effect,
        effect_size_H0     = current_effect_H0,
        norm_config        = norm_config,
        dist               = dist,
        gen_data           = gen_data,
        get_parameters     = get_parameters,
      ...
      #' -----------------------------------------------------------------------
      #' Build adaptive H1 p-values from stored normality decisions
      #' -----------------------------------------------------------------------
      use_test1_H1 <- vapply(
        sim_pvals$norm_decision_H1,
        use_parametric_test,
      ...
      #' -----------------------------------------------------------------------
      #' Compute power for each requested method
      #' -----------------------------------------------------------------------
      for (method in ds_test_methods) {
        
        results_power[[method]][i] <- mean(
          pval_H1_list[[method]] < test_alpha,
          na.rm = TRUE
        )
        
        pval_storage$H1[[method]][[i]] <- pval_H1_list[[method]]
      }
      
      timing[[paste0("d=", d)]] <- as.numeric(
  ...
  format_power_effect_results(
    results_by_dist = results_by_dist,
    distributions   = distributions,
...
run_simulation <- function(Nsim           = 1e3,
  ...
  if (run_phase(2)) {
    ...
    #' The inner perform_adaptive_analysis() call uses generate_pval(),
    #' so replicate-level work is handled inside generate_pval().
    #' ------------------------------------------------------------------
    if (per_n_thresholds) {
  ...
  ## Save only the structured results object.
  save(
    results,
    file     = rdata_results,
    compress = "xz"
  ...
  results$files$results_rdata <- rdata_results
  
  cat("  Saved results only:", rdata_results, "\n")
...
#' -- Threshold grids ----------------------------------------------------------
threshold_grid_classical <- seq(from = 0.00, to = 1, by = 0.005)
threshold_grid_ml        <- seq(from = 0.00, to = 1, by = 0.005)
...
#load("~/Desktop/OSU/Research Work/User_framework/ML_Application/ML_Application/trained_models.RData")


#load("trained_models/trained_models.RData")
...
#'
#' Performance note (single-model fast path):
#'   When use_majority_vote = FALSE and model_name is a single string
#'   (the typical Phases 2-6 configuration), this function calls
#'   ml_env$predict_prob_normal_fast() instead of classify_sample_prob().
#'   The fast path skips the data.frame result construction, list
#'   assembly, attribute tagging, and input-format detection that
#'   classify_sample_prob() performs for its general-purpose interface.
#'   When this function is called Nsim × N_tradeoff × n_distributions
#'   times those operations represent measurable overhead.
#'   The full classify_sample_prob() path is retained for majority-vote
#'   or multi-model calls where its output structure is required.
#' -----------------------------------------------------------------------------
...
onesample_bc_t_test_fns <- modifyList(
  onesample_fns,
  list(fn_for_ds_test_2 = boxcox_t_test)
)

#' -- Classical SW -------------------------------------------------------------
res_os_ttest_bc_t_test_sw <- do.call(run_simulation, c(
  onesample_params,
  list(
    test_type = "onesample_ttest_vs_bc_t_test_sw",
    norm_config = norm_config_sw,
    phases      = c(2, 3, 4, 5, 6)),
  onesample_bc_t_test_fns
))

#' -- Classical SW (per-n optimal thresholds) ----------------------------------
res_os_ttest_bc_t_test_sw_pern <- do.call(run_simulation, c(
  onesample_params,
  list(
    test_type        = "onesample_ttest_vs_bc_t_test_sw_pern",
    norm_config      = norm_config_sw,
    per_n_thresholds = TRUE,
    phases           = c(2, 3, 4, 5, 6)),
  onesample_bc_t_test_fns
...
#' ----------------------------------------------------------------------------
#' 5C. OLS regression vs MM-estimation robust regression
#' ----------------------------------------------------------------------------
regression_mm_fns <- modifyList(
  regression_fns,
  list(fn_for_ds_test_2 = mm_regression)
)
#' -- Classical SW -------------------------------------------------------------
res_reg_mm_sw <- do.call(run_simulation, c(
  regression_params,
  list(
    test_type   = "regression_ols_vs_mm_sw",
    norm_config = norm_config_sw,
    phases      = c(1, 2, 3, 4, 5, 6)),
  regression_mm_fns
))
#' -- Custom Fisher ------------------------------------------------------------
res_reg_mm_fisher <- do.call(run_simulation, c(
  regression_params,
  list(
    test_type   = "regression_ols_vs_mm_fisher",
    norm_config = norm_config_fisher,
    phases      = c(1, 2, 3, 4, 5, 6)),
  regression_mm_fns
))
#' -- ML: Phase 1 (all models compared) ---------------------------------------
res_reg_mm_ml_roc <- do.call(run_simulation, c(
  modifyList(regression_params, list(threshold_grid = threshold_grid_ml)),
  list(
    test_type   = "regression_ols_vs_mm_ml",
    norm_config = norm_config_ml_roc,
    phases      = c(1)),
  regression_mm_fns
))
#' -- ML: Phases 2-6 (chosen model after reviewing Phase 1) -------------------
res_reg_mm_ml <- do.call(run_simulation, c(
  modifyList(regression_params, list(threshold_grid = threshold_grid_ml)),
  list(
    test_type   = "regression_ols_vs_mm_ml",
    norm_config = norm_config_ml,
...
#' ----------------------------------------------------------------------------
#' 5D. OLS regression vs M-estimation robust regression
#' ----------------------------------------------------------------------------
regression_m_fns <- modifyList(
  regression_fns,
  list(fn_for_ds_test_2 = m_regression)
)
#' -- Classical SW -------------------------------------------------------------
res_reg_m_sw <- do.call(run_simulation, c(
  regression_params,
  list(
    test_type   = "regression_ols_vs_m_sw",
    norm_config = norm_config_sw,
    phases      = c(1, 2, 3, 4, 5, 6)),
  regression_m_fns
))
#' -- Custom Fisher ------------------------------------------------------------
res_reg_m_fisher <- do.call(run_simulation, c(
  regression_params,
  list(
    test_type   = "regression_ols_vs_m_fisher",
    norm_config = norm_config_fisher,
    phases      = c(1, 2, 3, 4, 5, 6)),
  regression_m_fns
))
#' -- ML: Phase 1 (all models compared) ---------------------------------------
res_reg_m_ml_roc <- do.call(run_simulation, c(
  modifyList(regression_params, list(threshold_grid = threshold_grid_ml)),
  list(
    test_type   = "regression_ols_vs_m_ml",
    norm_config = norm_config_ml_roc,
    phases      = c(1)),
  regression_m_fns
))
#' -- ML: Phases 2-6 (chosen model after reviewing Phase 1) -------------------
res_reg_m_ml <- do.call(run_simulation, c(
  modifyList(regression_params, list(threshold_grid = threshold_grid_ml)),
  list(
    test_type   = "regression_ols_vs_m_ml",
    norm_config = norm_config_ml,
